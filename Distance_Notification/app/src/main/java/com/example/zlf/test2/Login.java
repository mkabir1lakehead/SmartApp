...
public class Login extends Activity {                 //登录界面活动
    ...
    private TextView mCancleTextView;                     //注销按钮
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCancleTextView = (TextView) findViewById(R.id.login_btn_cancle);
        ...
        mCancleTextView.setOnClickListener(mListener);
    ...
    public void login() {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            ...
            int result=mUserDataManager.findUserByEmailAndPwd(userName, userPwd);
...
//                startActivity(intent);
                ...
                mPwd.setText("");
                mAccount.setText("");
                mUserDataManager.deleteUserDatabyEmail(userName);
            }else if(result==0){
    ...
    public boolean isUserNameAndPwdValid() {
        ...
        if (email.isEmpty() || email.length() < 3) {
            mAccount.setError("at least 3 characters");
            valid = false;
        } else {
        ...
        if (email.isEmpty() || !android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            mAccount.setError("enter a valid email address");
            valid = false;
        } else {
            mAccount.setError(null);
        }
        ...
        if (password.isEmpty() || password.length() < 4 || password.length() > 10) {
            mPwd.setError("between 4 and 10 alphanumeric characters");
            valid = false;
        } else {
            mPwd.setError(null);
        }
