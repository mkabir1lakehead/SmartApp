    ...
    private void setShowList() {
        ...
        for(int i=0;i<AppInfoList.size();i++) {
            if(AppInfoList.get(i).getUsedTimebyDay() > 0 ) { //&& AppInfoList.get(i).getTimes() > 0) {
                ...
                mostTimes = Math.max(  mostTimes, AppInfoList.get(i).getTimes());
                mostTime = Math.max(  mostTime, AppInfoList.get(i).getUsedTimebyDay());
            }
        }
...
//        }
    ...
    public long getMostTime() { return mostTime; }
